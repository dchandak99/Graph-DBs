...
for relat in relations1:
	...
	with open(dire + relat + '.csv', newline='') as csvfile:
		...
		for row in stud:
			...
			if(str1 == 'User'):
				userdict[idd] = not_idd
			if(str1 == 'Hashtag'):
				hashdict[idd] = not_idd
			if(str1 == 'Tweet'):
				tweetdict[idd] = not_idd
	...
	for row in stud:
		...
		for key, value in row.items():
			if(value == 'NULL'):
				...
				#print(key[5])
				try:
					qu1 = qu1 + ' (' + 'user' + str(i) + ': ' + 'User' + '{ ' + 'name' + ': ' + '\"' + userdict[int(value)] + '\"' + '}),'                         
				except ValueError:
					qu1 = qu1 + ' (' + 'user' + str(i) + ': ' + 'User' + '{ ' + 'name' + ': ' + '\"' + value + '\"' + '}),'      
	...
	for row in stud:
		...
		for key, value in row.items():
			if(value == 'NULL'):
				...
				if(i == 1):
					try:
						qu1 = qu1 + ' (' + 'user' + ': ' + 'User' + '{ ' + 'name' + ': ' + '\"' + userdict[int(value)] + '\"' + '}),'                         
					except ValueError:
						qu1 = qu1 + ' (' + 'user' + ': ' + 'User' + '{ ' + 'name' + ': ' + '\"' + value + '\"' + '}),'      
					
				if(i == 2):
					try:
						qu1 = qu1 + ' (' + 'tweet' + ': ' + 'Tweet' + '{ ' + 'text' + ': ' + '\"' + tweetdict[int(value)] + '\"' + '})'                         
					except ValueError:
						qu1 = qu1 + ' (' + 'tweet' + ': ' + 'Tweet' + '{ ' + 'text' + ': ' + '\"' + value + '\"' + '})' 
	...
	for row in stud:
		...
		for key, value in row.items():
			if(value == 'NULL'):
				...
				if(i == 2):
					try:
						qu1 = qu1 + ' (' + 'hashtag' + ': ' + 'Hashtag' + '{ ' + 'tag' + ': ' + '\"' + hashdict[int(value)] + '\"' + '})'                         
					except ValueError:
						qu1 = qu1 + ' (' + 'hashtag' + ': ' + 'Hashtag' + '{ ' + 'tag' + ': ' + '\"' + value + '\"' + '})'      
